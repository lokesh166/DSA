...
public:
    ...
    bool solve(int i,int j,int length,vector<vector<char>> &board,string &word){
        ...
        if(!isValid(i,j,board) or board[i][j] == '$' or board[i][j] != word[length]) return false;
        
        char ch = board[i][j];
        board[i][j] = '$';
        
        bool res = false;
        if(!res) res = solve(i + 1 , j , length + 1 , board , word);
        if(!res) res = solve(i - 1 , j , length + 1 , board , word);
        if(!res) res = solve(i , j + 1 , length + 1 , board , word);
        if(!res) res = solve(i , j - 1 , length + 1 , board , word);
